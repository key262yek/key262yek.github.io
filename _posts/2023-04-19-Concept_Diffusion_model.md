...
따라서 $w \times h$ 크기 이미지는 $\qty{0, 1, \cdots, 255}^{3wh}$의 원소일 것입니다.
...
분자는 $N$개 종류의 원자들이 node가 되고, 결합을 edge로 하는 임의의 사이즈를 가진 네트워크로 볼 수 있으므로 원자 개수 $n$개의 분자는 $\qty{ 1, \cdots, N }^n \qty{ 0, 1, 2, 3 }^{n(n - 1)/2}$의 원소가 됩니다.

그렇다면 공간 $[0, 1]^{3wh}$의 모든 벡터는 이미지라고 할 수 있을까요?
$\mathbb{R}_+^{T}$의 모든 원소는 소리일까요?
$\qty{ 1, \cdots, N }^n \qty{ 0, 1, 2, 3 }^{n(n - 1)/2}$의 원소는 항상 어떤 분자에 대응된다고 할 수 있을까요?
그렇지 않을겁니다.
...
**After diffusion**
{% include plots/fully_noised_data.html %}
...
\begin{equation}
  ...
  \mathbb{E}_q \qty[- \log p(x_T) - \sum_{t \geq 1} \log \frac{p(x_{t-1} | x_t)}{q(x_t | x_{t-1})}]
\end{equation}
$$
...
\begin{equation}
  \dd X(t) = - \frac{1}{\gamma} \nabla V(X) \dd t + \sqrt{2 D} \dd W 
\end{equation}

를 만족하며 변하는 입자의 평형 분포 $P(x)$가 아래와 같이 주어짐을 이용하는 샘플링 방법론입니다.

\begin{equation}
  P(x) \propto \exp\qty(- \frac{V(x)}{k_B T}), \quad \gamma D = 2 k_B T
\end{equation}
...
\begin{equation}
  \dd X(t) = \frac{1}{\gamma} \nabla \log P(x) \dd t + \sqrt{2 D} \dd W 
\end{equation}
